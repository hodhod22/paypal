...
const ConfirmOrder = () => {
  ...
  const price = localStorage.getItem("thisPrice");
  const accountNum = localStorage.getItem("thisAccountnumber");
  ...
  const update_payment = async () => {
    ...
    if (orderId) {
      try {
        await axios.post(`http://localhost:5000/api/confirm/${orderId}`, {
          price,
          accountNum,
        });
